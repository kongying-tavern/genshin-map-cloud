...
public class DiffUtils {
    ...
    public static List<FieldDiff> getFieldsDiff(Object before, Object after, FieldDiffConfig config) {
        ...
        if (CollectionUtil.isEmpty(cfDiffFields)) {
            ...
            List<String> dtBeforeFieldNames = dtBeforeFields.stream().filter(Objects::nonNull).map(Field::getName)
                .collect(Collectors.toList());
            List<Field> dtAfterFields = ClassUtils.getFields(after.getClass());
            List<String> dtAfterFieldNames = dtAfterFields.stream().filter(Objects::nonNull).map(Field::getName)
                .collect(Collectors.toList());
            cfDiffFields = CollectionUtil.union(dtBeforeFieldNames, dtAfterFieldNames).stream().filter(v -> v != null)
            ...
            for (int i = 1; i <= s1Len; i++) {
                ...
                for (int j = 1; j <= s2Len; j++) {
                    ...
                    if (s1Char.equals(s2Char)) {
                        ...
                        distance[i][j] = Math
                            .min(Math.min(distance[i - 1][j], distance[i][j - 1]), distance[i - 1][j - 1]) + 1;
                    }
                }
            }
