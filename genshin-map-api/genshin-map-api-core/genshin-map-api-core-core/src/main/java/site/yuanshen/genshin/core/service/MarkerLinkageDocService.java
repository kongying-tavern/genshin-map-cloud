...
public class MarkerLinkageDocService {
     ...
     */
    @Cacheable(
        value = MarkerLinkageCacheKeyConst.MARKER_LINKAGE_LIST_BIN_MD5, cacheManager = "neverRefreshCacheManager"
    )
     ...
     */
    @Cacheable(
        value = MarkerLinkageCacheKeyConst.MARKER_LINKAGE_GRAPH_BIN_MD5, cacheManager = "neverRefreshCacheManager"
    )
    ...
    public BinaryMD5Vo refreshMarkerLinkageGraphBinaryMD5() {
        ...
        log.info(
            "点位关联有向图MD5生成, cost:{}, result: {}", System.currentTimeMillis() - startTime, JSON.toJSONString(result)
        );
        BinaryMD5Vo binaryMD5Vo = new BinaryMD5Vo();
        binaryMD5Vo.setMd5(result);
        binaryMD5Vo.setTime(time);
