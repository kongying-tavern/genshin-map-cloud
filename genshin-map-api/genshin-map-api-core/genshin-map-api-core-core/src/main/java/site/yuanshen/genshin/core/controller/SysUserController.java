...
public class SysUserController {
    ...
    public R<Long> registerUser(@RequestBody SysUserRegisterVo registerVo) {
        if (!UserUtils.checkRegisterParamEmpty(registerVo))
            throw new GenshinApiException("请检查注册参数，不允许空用户名或者空密码");
    ...
    public R<Long> registerUserByQQ(@RequestBody SysUserRegisterVo registerVo) {
        if (!checkRegisterParamEmpty(registerVo))
            throw new GenshinApiException("请检查注册参数，不允许空qq号或者空密码");
        if (!checkRegisterQQParam(registerVo))
            throw new GenshinApiException("qq号为空或格式不匹配");
    ...
    public R<Boolean> updateUser(
        @RequestBody SysUserUpdateVo updateVo,
        @Parameter(hidden = true) @RequestHeader("userId") Long headerUserId,
        @Parameter(hidden = true) @RequestHeader("Authorities") String rolesString
    ) {
    ...
    public R<Boolean> updateUserPassword(
        @RequestBody SysUserPasswordUpdateVo updateVo,
        @Parameter(hidden = true) @RequestHeader("userId") Long headerUserId
    ) {
