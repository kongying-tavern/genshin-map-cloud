package site.yuanshen.genshin.core.service.mbp.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import org.springframework.stereotype.Service;
import site.yuanshen.data.entity.MarkerExtra;
import site.yuanshen.data.mapper.MarkerExtraMapper;
import site.yuanshen.genshin.core.service.mbp.MarkerExtraMBPService;

/**
 * 点位额外字段表 Mybatis Plus CRUD服务实现类
 *
 * @author Moment
 * @since 2022-06-25 12:17:27
 */
@Service
public class MarkerExtraMBPServiceImpl extends ServiceImpl<MarkerExtraMapper, MarkerExtra> implements MarkerExtraMBPService {

}
