...
public class RouteController {
    ...
    public R<PageListVo<RouteVo>> listRoutePage(
    ...
    public R<PageListVo<RouteVo>> listRoutePageSearch(
        @RequestBody RouteSearchVo searchVo,
        @Parameter(hidden = true) @RequestHeader(value = "userDataLevel", required = false) String userDataLevel
    ) {
    ...
    public R<List<RouteVo>> listRouteById(
    ...
    @PutMapping("/add")
    public R<Long> createRoute(
        @RequestBody RouteVo routeVo, @Parameter(hidden = true) @RequestHeader("userId") Long userId
    ) {
    ...
    @PostMapping
    public R<Boolean> updateRoute(
        @RequestBody RouteVo routeVo, @Parameter(hidden = true) @RequestHeader("userId") Long userId,
        @Parameter(hidden = true) @RequestHeader("Authorities") String authoritiesString
    ) {
    ...
    public R<Boolean> deleteRoute(
        @PathVariable("routeId") Long routeId, @Parameter(hidden = true) @RequestHeader("userId") Long userId,
        @Parameter(hidden = true) @RequestHeader("Authorities") String authoritiesString
    ) {
        RouteDto route = routeService.listRouteById(Collections.singletonList(routeId), HiddenFlagEnum.getAllFlagList())
            .get(0);
    ...
    private void checkRole(Long userId, Long headerUserId, String authoritiesString) {
        List<RoleEnum> userRoleList = JSON.parseArray(authoritiesString).toJavaList(String.class).stream()
            .map(RoleEnum::valueOf).collect(Collectors.toList());
        for (RoleEnum role : userRoleList) {
            if (role.compareTo(RoleEnum.MAP_MANAGER) >= 0)
                return;
        }
        if (!userId.equals(headerUserId))
            throw new GenshinApiException("无权限进行此路线操作，请检查路线所有者是否为当前用户");
    }
