...
public class PunctuateController {
    ...
    @Operation(
        summary = "分页查询自己提交的未通过的打点信息",
        description = "分页查询自己提交的未通过的打点信息（打点员的API）"
    )
    @PostMapping("/get/page/{authorId}")
    public R<PageListVo<MarkerPunctuateVo>> listSelfPunctuatePage(
        @RequestBody PageSearchVo pageSearchVo, @PathVariable("authorId") Long authorId,
        @Parameter(hidden = true) @RequestHeader("userId") Long userId
    ) {
        if (!userId.equals(authorId))
            throw new GenshinApiException("无权限查看其他人未提交的打点信息");
        return RUtils.create(
    ...
    @PutMapping("/")
    public R<Long> addPunctuate(
        @RequestBody MarkerPunctuateVo punctuateVo, @Parameter(hidden = true) @RequestHeader("userId") Long userId
    ) {
        ...
        if (punctuateVo.getOriginalMarkerId() == null && !punctuateVo.getMarkerCreatorId().equals(userId))
            throw new GenshinApiException("新增点位时，点位初始标记者需为用户自身");
        return RUtils.create(
    ...
    public R<Boolean> pushPunctuate(
        @PathVariable("authorId") Long authorId, @Parameter(hidden = true) @RequestHeader("userId") Long userId
    ) {
        if (!userId.equals(authorId))
            throw new GenshinApiException("无权限将其他用户的暂存点位提交审核");
        return RUtils.create(
    ...
    @PostMapping("/")
    public R<Boolean> updateSelfPunctuate(
        @RequestBody MarkerPunctuateVo markerPunctuateVo, @Parameter(hidden = true) @RequestHeader("userId") Long userId
    ) {
        if (!userId.equals(markerPunctuateVo.getAuthor()))
            throw new GenshinApiException("无权限修改其他人未提交的打点信息");
        return RUtils.create(
    ...
    public R<Boolean> deleteSelfPunctuate(
        @PathVariable("punctuateId") Long punctuateId, @PathVariable("authorId") Long authorId,
        @Parameter(hidden = true) @RequestHeader("userId") Long userId
    ) {
        if (!userId.equals(authorId))
            throw new GenshinApiException("无权限删除其他用户的暂存点位");
        return RUtils.create(
